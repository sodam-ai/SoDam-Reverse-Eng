...
| `/re-binary` (실행파일 분석) | Java 17+, Ghidra **또는** Python+LIEF(경량 대안) | §2 |
...
> ⚠️ 골격은 완료돼 있으나, 이 경로는 아직 라이브 검증 전입니다(Ghidra 미설치 상태로 개발됨).

### 방법 A — Ghidra (정식, 무료, NSA 개발)

- **공식 배포처**: [ghidra-sre.org](https://ghidra-sre.org/)
- **필요 조건**: Java 17 이상(§1-1과 동일)
- **설치**: 압축 해제 후 `ghidraRun.bat`(Windows) 실행 확인
- **자주 겪는 문제**: `ghidraRun`이 안 열리면 대부분 `JAVA_HOME` 환경변수 미설정이 원인 — Java 설치 경로를 `JAVA_HOME`으로 지정 후 새 터미널에서 재시도
