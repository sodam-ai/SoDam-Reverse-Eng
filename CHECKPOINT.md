...
- **브랜치:** `feat/m5-readiness` — `origin/feat/m5-readiness` 대비 **7커밋 앞섬(2026-07-11 재확인), 미푸시**. 작업 트리 clean, 보호5파일 diff 0.
...
- **다음 단계 → §6 참조. 지금 즉시 실행 가능한 것은 NEXT-3(GitHub 백업 푸시) 하나뿐** — 나머지(NEXT-2/2b/4b/5)는 도구미설치·상용라이선스·보호파일·정책보류로 구조적 차단.
- **다음 할 일 → §6 "다음 작업" 참조.**
...
| **NEXT-3** | **GitHub 백업 푸시**(`feat/m5-readiness`→`origin`, main 무변경) | 사용자 결정 | 사전점검 전부 완료 확인됨(2026-07-11 재검증: 작업트리 clean·보호5파일 diff 0·`.PRD` 미추적·origin 대비 ahead 7) — **남은 건 실행 승인뿐, 지금 유일하게 즉시 실행 가능한 항목** |
...
| **NEXT-4b** | **보호파일 버그 2건 수정**(§5-1 참조) | 사용자 수동 | `re-deny-guard.mjs` 빈입력 fail-open + 훅 중복등록 — 수정안 텍스트 이미 §5-1에 기록됨, `SETUP_BLOCKED_FILES.md` 경로로 적용 후 `integrity.json` 해시 재기록 필요. **NEXT-3(푸시)의 선행조건 아님**(독립적) |
...
**NEXT-3 — GitHub 백업 푸시 (사용자 결정, 지금 실행 가능)**
- **리스크:** `push`는 **비가역/대외**(비공개 저장소라도) → **승인 필수**. 단, 사전점검(작업트리 clean·`git ls-files .PRD`빈값·보호5파일 diff 0)은 **2026-07-11 재확인 완료** — 추가 대기 사유 없음. main 머지가 아니라 **feature 브랜치만** 푸시(백업 목적, 되돌리기 쉬움).
- **NEXT-4b와의 관계:** §5-1의 보호파일 버그 2건은 이 푸시를 막지 않음(별개 트랙) — 원하면 나중에 별도로 수정·재검증·커밋 가능.

**NEXT-4b — 보호파일 버그 2건 수정 (사용자 수동, 낮은 긴급도)**
- **리스크:** 실사용 중 재현 확률 낮음(정상 하네스는 빈 stdin을 안 보냄, 훅 중복은 오작동 아닌 비효율) — 그래도 PRD MUST 요구사항 위반이라 방치 금지 원칙엔 어긋남. 수정 시 `_selftest.mjs` 재실행 → `integrity.json` 해시 재기록 필수(안 하면 3층 무결성 검사가 "변조"로 오탐).
