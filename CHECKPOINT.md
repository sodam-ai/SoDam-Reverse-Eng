...
## 5-2. 알려진 한계 — 셀프테스트 커버리지 공백 (2026-07-11 기록)

> `hooks/_selftest.mjs`는 보호파일이라 AI가 직접 편집할 수 없다. `re-analyze-agent`·`re-analyze-binary` 스킬에 새로 추가된 §0-1 규칙(대상 파일 안의 위장 지시문·가짜 동의 기록을 걸러내는 1층 규칙)은 이 셀프테스트가 검증하지 않는다 — 기존 3개 스킬(`re-router`/`re-analyze-mycode`/`re-report`)만 대상으로 작성됐기 때문.
> **"셀프테스트 6/0 통과"는 안전 3층의 원래 항목(1층 기본규칙·2층 deny-hook·3층 무결성)만 확인한다 — 신규 1층 규칙까지 포함한 완전 검증으로 오인하지 말 것.** 커버리지 확장은 `_selftest.mjs` 수정이 필요해 사람 몫(`SETUP_BLOCKED_FILES.md` 경로).
