...
- 분석 중 **시크릿(키·토큰·비밀번호)을 발견하면 보고서·로그에 평문으로 쓰지 말고 마스킹**한다 (`references/mask-patterns.json` 참조). **매칭된 문자열 전체를 `mask-patterns.json`의 `_mask_with`(`••••(마스킹됨)`) 값으로 완전히 치환한다 — 앞/뒤 일부 글자도 남기지 않는다**(예: `sk-a••••7890`처럼 접두/접미를 남기는 부분 마스킹 금지).
