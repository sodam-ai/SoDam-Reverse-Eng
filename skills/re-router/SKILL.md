...
2·3을 고르면: "아직 준비 중이며, 지금은 '내 코드/앱' 또는 'AI 코딩 에이전트 구조 분석'이 가능합니다"라고 안내한다.

## 2. 동의 게이트 (분석 전 필수 — 통과 못 하면 분석 0건)

분석 대상 경로를 확인하고, 다음 **세 가지에 명시적 '예'**를 받는다(re-android·re-binary와 동일한 3문항 구조로 통일, 2026-07-27):

- "이 대상은 **본인 소유** 또는 **분석 허가를 받은** 것이 맞나요?"
- "이 분석은 **방어·교육 목적**이 맞나요?"
- "결과는 참고용이며, 안전/위험을 단정하지 않고 **책임은 사용자에게** 있음을 이해하셨나요?"

셋 다 '예'가 아니면 **중단**한다. 동의는 **대상 경로 단위**로 기록한다(02_DATA_MODEL의 ConsentRecord: target_scope, ownership, disclaimer_ack).
