...
사용자 의도를 확인한다. **현재 1·2·3·4번 전부 활성**(2026-09-11 갱신 — 바이너리 RE도 2026-08-21 실사용 라이브 종단 검증 완료가 실제 Ghidra 분석 성공(함수 추출까지)으로 확인됨. 이 갱신 전까지는 §0-1(레드팀 검증된 안전장치)이 완료된 기능인데도 자연어 라우팅 경로만 막혀 있었음 — 슬래시 명령 `/sodam-reverse:re-binary`로는 이미 정상 작동 중이었음):

1. **내 코드/앱 이해·문서화** ✅ (지금 가능) → `re-analyze-mycode` 스킬로 진행
2. **안드로이드 앱 분석(APK)** ✅ (지금 가능) → `re-analyze-android` 스킬로 진행
3. **바이너리 RE (Ghidra)** ✅ (지금 가능) → `re-analyze-binary` 스킬로 진행
